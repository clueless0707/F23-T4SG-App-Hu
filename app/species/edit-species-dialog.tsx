...
const EditSpeciesDialog: React.FC<SpeciesCardProps> = ({ species }) => {
  ...
  const onSubmit = async (input: FormData) => {
    ...
    const { error } = await supabase
      ...
      .match({ id: species.id }); // Condition to match the record to update
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-screen overflow-y-auto sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Edit the species information. Click &quot;Update Species&quot; below when you&apos;re done.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={(e: BaseSyntheticEvent) => void form.handleSubmit(onSubmit)(e)}>
            <div className="grid w-full items-center gap-4">
              <FormField
                ...
                render={({ field }) => {
                  // We must extract value from field and convert a potential defaultValue of `null` to ""
                  // because inputs can't handle null values: https://github.com/orgs/react-hook-form/discussions/4091
                  ...
                  return (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          value={value ?? species.scientific_name ?? ""}
                          {...rest}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue defaultValue={"Select Kingdom Value"} />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                        <SelectGroup>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      ...
                      <Input
                        type="number"
                        defaultValue={species.total_population?.toString()}
                        {...field}
                        onChange={(event) => field.onChange(+event.target.value)}
                      />
                    </FormControl>
              ...
              <FormField
                ...
                render={({ field }) => {
                  // We must extract value from field and convert a potential defaultValue of `null` to ""
                  // because textareas can't handle null values: https://github.com/orgs/react-hook-form/discussions/4091
                  const { value, ...rest } = field;
                  return (
                    <FormItem>
                      <FormLabel>Description</FormLabel>
                      <FormControl>
                        <Textarea value={value ?? species.description ?? ""} {...rest} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  );
